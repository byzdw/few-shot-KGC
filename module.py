...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_seq_len):
        ...
        """
        # 不要cos和sin编码，一共就三个值没必要又绝对位置又相对位置
        # 1。只用sin
        # """
        position_encoding = np.array([
            [pos / np.power(10000, j / d_model) for j in range(d_model)]
            for pos in range(max_seq_len)])

        position_encoding = np.sin(position_encoding)
        """结束"""
        ...
        # """
        # position_encoding = np.array([
        #     [0.5 * (pos+1) * math.pi for j in range(d_model) ] for pos in range(max_seq_len)])
        # position_encoding = np.sin(position_encoding)
        # """"""
